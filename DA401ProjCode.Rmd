...
ggsave("GSDist.png", GameScoreDist, "png", "C:\\Users\\jcald\\OneDrive\\Documents\\DA-401\\Visuals\\", dpi = 300)
ggsave("PERDist.png", PERDist, "png", "C:\\Users\\jcald\\OneDrive\\Documents\\DA-401\\Visuals\\", dpi=300)
```
...
relPlot <- ggplot(data = merge(CareerAvgGameScore, CareerAvgPER, by = "Player"), aes(x = CareerAvgGameScore$GameScore, y = CareerAvgPER$PER))+
  geom_point()+
  geom_hline(yintercept = 17, col = "red")+
...
PERGSVenn <- ggVennDiagram(x, label = "count", label_alpha = 0.4, category.names = c("PER", "GameScore", "NBA 75"))+
...
PERVIP <- ggplot(melt(imp, id.vars="rn"), 
    ...
    facet_wrap(~variable) +
    theme_classic() +
    labs(title = "Variable Importance plot for PER Random Forest",y="Percent", x=NULL)
...
GSVIP <- ggplot(melt(imp, id.vars="rn"), 
    ...
    facet_wrap(~variable) +
    theme_classic() +
    labs(title = "Variable Importance plot for GameScore Random Forest",y="Percent", x=NULL)
...
SProdDist <- NBA %>% ggplot(aes(x=SProd))+
  geom_histogram(bins = 10, fill = 'blue', color = '#000000', aes(y = ..density..))+
  geom_density(fill = "#F85700", color = "#000000", alpha = 0.6)+
  ...
  theme_classic()+
theme(title = element_text(size = 20), axis.title.x = element_text(size = 16), axis.title.y = element_text(size = 16), axis.text = element_text(size = 12))
ggsave("SProdDist.png", SProdDist, "png", "C:\\Users\\jcald\\OneDrive\\Documents\\DA-401\\Visuals\\", dpi=300)
```
...
SPRODVenn <- ggVennDiagram(x, label = "count", label_alpha = 0.4, category.names = c("SProd", "NBA 75"))+
             scale_x_continuous(expand = expansion(mult = .2))+
             scale_y_continuous(expand = expansion(mult = .2))+
             ggplot2::scale_fill_gradient(low="yellow",high = "dark green")
ggsave("SPRODVenn.png", SPRODVenn, "png", "C:\\Users\\jcald\\OneDrive\\Documents\\DA-401\\Visuals", dpi=300)
setwd("C:\\Users\\jcald\\OneDrive\\Documents\\DA-401\\Data\\")
RankedSProd <- read.csv("Top50SeasonProduction.csv")
RankedFamiliarity <- read.csv("RankedFamiliarity.csv")
```
